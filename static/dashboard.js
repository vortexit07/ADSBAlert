...
async function confirmQuit() {
    if (confirm('Stop the ADSBAlert app? You can restart it from the command line.')) {
        try {
...
async function fetchStatus() {
    try {
        ...
        if (typeof s.last_fetch_time === 'number' && !Number.isNaN(s.last_fetch_time)) {
...
async function loadData() {
    ...
    data.forEach(ac => {
        ...
        row.innerHTML = `
            <td>${ac.icao24}</td>
            <td>${ac.callsign || ac.registration}</td>
